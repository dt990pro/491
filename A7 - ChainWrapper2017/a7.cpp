...
const char *WINDOWTITLE = { "Jiawei Mo" };
...
Display( void )
{
	...
	// show figs.
	{
		char x[128], y[128], mass[128], spring_const[128], damping[128], g[128], dt[128], ord[128];
		sprintf(x, "X0: %.3f", X0);
		sprintf(y, "Y0: %.3f", Y0);
		sprintf(mass, "Mass: %.3f", Mass);
		sprintf(spring_const, "Spring Constant: %.3f", K);
		sprintf(damping, "Damping: %.3f", Cd);
		sprintf(g, "Gravity: %.3f", Gravity);
		sprintf(dt, "Dt: %.3f", Dt);
		sprintf(ord, "Order: %d", Order);
		...
		glLoadIdentity();
		glColor3f(1., 1., 1.);
		DoRasterString(5., 85., 0., x);
		DoRasterString(5., 80., 0., y);
		DoRasterString(5., 75., 0., mass);
		DoRasterString(5., 70., 0., spring_const);
		DoRasterString(5., 65., 0., damping);
		DoRasterString(5., 60., 0., g);
		DoRasterString(5., 55., 0., dt);
		DoRasterString(5., 50., 0., ord);
	}
...
GetDerivs( struct state state[NUMNODES], struct derivatives derivs[NUMNODES] )
{
	for( int node = 0; node < NUMNODES; node++ )
	{
		...
		float length = sqrtf(xm * xm + ym * ym);		// get dL
		float stretch = length - LENGTH0;				// amount spring is stretched
		float force = K * stretch;
		sumfx += force * xm / length;		// cos(theta)
		sumfy += force * ym / length;		// sin(theta)

		if( node < NUMNODES-1 )		// if has next node
		{
			xp = state[node+1].x - state[node].x;
			yp = state[node+1].y - state[node].y;
			length = sqrtf(xp * xp + yp * yp);		// get dL
			stretch = length - LENGTH0;				// amount spring is stretched
			force = K * stretch;
			sumfx += force * xp / length;		// cos(theta)
			sumfy += force * yp / length;		// sin(theta)
		}

		float v = sqrt( state[node].vx*state[node].vx + state[node].vy*state[node].vy );
		if( v > 0. )	// if has air resistance
		{
			sumfx -= Cd * state[node].vx;		// ??????????
			sumfy -= Cd * state[node].vx;		// ??????????
		}
